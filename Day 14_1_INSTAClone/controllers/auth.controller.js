...
//* and return krdega jiske basis pe user already exist
    const isUserAlreadyExists = await userModel.findOne({ //? yeh function check krrega
        $or: [
            {username} , { email}
        ]
    }) 
    if(isUserAlreadyExists){
        ...
        json({
            message: "User already exist" + isUserAlreadyExists.email == email 
...
async function loginController(req,res){
    ...
    //* user here can login in two way-
        ...
        $or:[ //yeh array hote conditions ke jinme conditions likhte hum
            {
               //condition 1
                username: username
