...
// TODO>  User jab bhi register krega usse Token diya jaega
...
// !POST /api/auth/login 
//  *Controller >>> wo sabhi functions yaah callback jo sirf tabhi chlte jab unke api pe request aati unko controller khte
// TODO>> User jab bhi login krega usse new token diya jaaega
authRouter.post("login",async (req,res)=>{
    const { email, password } = req.body
    const user = await userModel.findOne({email}) //? yeh line databse se mila hua user document return krrhi
    if(!user){
        return res.status(404).json({
            message:"Not found!"
        })
    }
    const isPasswordMatched = user.password === crypto.createHash("md5").update(password).digest("hex") // TODO> req.body ka pass hai yeh
...
authRouter.get('/get-me',async(req,res)=>{
    const token = req.cookies.token //iski help se token mil jaega hamara
    const decoded = jwt.verify(token,process.env.JWT_SECRET) // tokken jo hai hmare server ka hi jwt verify krte
    // token shi milega to decode main user detail menas id milegi jiske basis pe humne token bnya tha
    const user =  await userModel.findById(decpded.id)
